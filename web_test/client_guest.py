...
async def main() -> None:
    ...
    room = input("room_code: ").strip().upper()
    ...
    print("[guest] welcome:", welcome)

    await send(writer, {"type": "join_room", "room": room})
    joined = json.loads((await reader.readline()).decode("utf-8", errors="replace"))
    print("[guest] join:", joined)
